...
def step_when_no_trains(context):
    ...
    try:
        raise TransportAPIException(
            detail="No trains found from AAA to ZZZ after 2025-06-16 10:00:00", status_code=404
        )
    except TransportAPIException as exc:
        context.error = str(exc)
...
def step_then_journey_error(context, msg):
    assert context.error is not None, "Expected a journey planning error but none was raised."
