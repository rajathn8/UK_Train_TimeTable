...
def add_train_service(db: Session, service_id: str, operator: str, operator_name: str) -> TrainService:
    service = TrainService(service_id=service_id, operator=operator, operator_name=operator_name)
    db.add(service)
    db.commit()
    db.refresh(service)
    return service

def add_timetable_entry(db: Session, service_id: str, station_from: str, station_to: str, aimed_departure_time: datetime, aimed_arrival_time: datetime) -> TimetableEntry:
...
def get_timetable_entries(db: Session, station_from: str, station_to: str, after_time: datetime) -> List[TimetableEntry]:
    ...
    ).order_by(TimetableEntry.aimed_departure_time).all()
